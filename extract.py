...
def card_ID(packet_string):
    ...
    Returns :
                    card_id (int) : card ID in decimal
    '''
    return(int(packet_string[24:32],16))
...
def what_operation(packet_string):
    ...
    '''
    if (start_credit_value(packet_string) > end_credit_value(packet_string)):
        return "Top up"
    elif (start_credit_value(packet_string) < end_credit_value(packet_string)):
...
def flow(packet_string):
    ...
    Returns :
                    (int) : flow time 
    '''
    return(liters(packet_string)/flow_time(packet_string))
